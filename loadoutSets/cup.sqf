HelperDroneClass = "B_UAV_06_F";
EscapeHeliClass = "CUP_B_UH60M_FFV_US";
FriendlyGroupClasses = [
	[
		"CUP_B_US_Soldier_TL_OCP",
		"CUP_B_US_Soldier_GL_OCP",
		"CUP_B_US_Soldier_Marksman_OCP",
		"CUP_B_US_Medic_OCP"
	],
	[
		"CUP_B_US_Soldier_TL_OCP",
		"CUP_B_US_Soldier_MG_OCP",
		"CUP_B_US_Soldier_GL_OCP",
		"CUP_B_US_Soldier_AMG_OCP"
	],
	[
		"CUP_B_US_Soldier_TL_OEFCP",
		"CUP_B_US_Soldier_AR_OEFCP",
		"CUP_B_US_Soldier_GL_OEFCP",
		"CUP_B_US_Soldier_AAR_OEFCP"
	]
];

EnemyAAClass = "CUP_O_ZSU23_SLA";
EnemyMortarClass = "CUP_O_2b14_82mm_SLA";
EnemyPlaneClass = "CUP_O_Su25_Dyn_SLA";
EnemyPlaneWeapon = "CUP_Vblauncher_Fab250_veh";
EnemyPlaneMode = "CUP_Vblauncher_Fab250_veh";
EnemyReconHeliClass = "CUP_O_UH1H_SLA";
EnemyChuteHeliClass = "CUP_O_Mi8_SLA_1";

EnemyParatroopers = [
	"CUP_O_RU_Officer_VDV_EMR",
	"CUP_O_RU_Medic_VDV_EMR",
	"CUP_O_RU_Soldier_AR_VDV_EMR",
	"CUP_O_RU_Soldier_AR_VDV_EMR",
	"CUP_O_RU_Sniper_VDV_EMR"
];

EnemyATGroups = [
	[
		"CUP_O_sla_Soldier_HAT",
		"CUP_O_sla_Soldier_HAT",
		"CUP_O_sla_Soldier_AT",
		"CUP_O_sla_Soldier_AT",
		"CUP_O_sla_Soldier_AAT"
	]
];
EnemyVehicles = ["CUP_O_UAZ_AGS30_SLA", "CUP_O_BRDM2_SLA"];
EnemyBattleHelis = ["CUP_O_Ka50_DL_SLA", "CUP_O_sla_Pilot"];
HeavySquads = [
	[
		"CUP_O_sla_SpecOps_TL",
		"CUP_O_sla_SpecOps_MG",
		"CUP_O_sla_SpecOps",
		"CUP_O_sla_SpecOps",
		"CUP_O_sla_SpecOps_MG",
		"CUP_O_sla_SpecOps",
		"CUP_O_sla_SpecOps"
	],
	[
		"CUP_O_sla_Soldier_SL",
		"CUP_O_sla_Soldier_MG",
		"CUP_O_sla_Soldier_AT",
		"CUP_O_sla_Soldier_GL",
		"CUP_O_sla_Soldier_MG",
		"CUP_O_sla_Soldier_LAT",
		"CUP_O_sla_Soldier_GL",
		"CUP_O_sla_Soldier_AAT",
		"CUP_O_sla_Soldier_AMG"
	],
	[
		"CUP_O_Partisans_soldier_SL",
		"CUP_O_Partisans_Soldier_MG",
		"CUP_O_Partisans_Soldier_AT",
		"CUP_O_Partisans_soldier_TTsKO",
		"CUP_O_Partisans_Soldier_MG",
		"CUP_O_Partisans_Soldier_AT",
		"CUP_O_Partisans_Medic",
		"CUP_O_Partisans_soldier_TTsKO",
		"CUP_O_Partisans_Engineer"
	]
];
LightSquads = [
	[
		"CUP_O_sla_Soldier_HAT",
		"CUP_O_sla_Soldier_AT",
		"CUP_O_sla_Soldier_AT",
		"CUP_O_sla_Soldier_AAT"
	],
	[
		"CUP_O_sla_Soldier_MG",
		"CUP_O_sla_Soldier_GL",
		"CUP_O_sla_Soldier",
		"CUP_O_sla_Soldier"
	],
	[
		"CUP_O_sla_Soldier",
		"CUP_O_sla_Soldier",
		"CUP_O_sla_Soldier_AA",
		"CUP_O_sla_Soldier_AA"
	],
	[
		"CUP_O_sla_Sniper",
		"CUP_O_SLA_Spotter"
	],
	[
		"CUP_O_Partisans_soldier_SL",
		"CUP_O_Partisans_Soldier_MG",
		"CUP_O_Partisans_soldier_TTsKO",
		"CUP_O_Partisans_soldier_TTsKO"
	],
	[
		"CUP_O_sla_Soldier_MG_urban",
		"CUP_O_sla_Soldier_GL_urban",
		"CUP_O_sla_Soldier_urban",
		"CUP_O_sla_Soldier_urban"
	]

];

CivCars = [
	"CUP_C_Golf4_random_Civ",
	"CUP_C_Octavia_CIV",
	"C_SUV_01_F",
	"C_Van_01_transport_F",
	"C_Truck_02_transport_F"
]